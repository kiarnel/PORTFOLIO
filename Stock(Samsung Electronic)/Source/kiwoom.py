...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        #시간 관련 변수
        self.now1 = QDate.currentDate()
        self.date1 = self.now1.toString('yyyyMMdd')
        ...
        self.screen_commrqdata_stock = "3000"
        ...
        self.day_receive_stock_info()  # 일봉데이터 조회 얻어오기
        # self.judge_stock()
        self.menu()
    ...
    def menu(self):
        ...
        while True:
            ...
            if sel == "1":
                ...
                self.print_get_day_account_info()
            # elif sel == "7":
            #     self.judge_stock()
    ...
    # 현재가
    def print_get_day_account_info(self):
        os.system('cls')
        df_day = pd.DataFrame(self.calculator_list, columns=['현재가', '거래량', '거래대금', '일자', '시가', '고가', '저가'])
        print(df_day)
        # col_df_day = df_day['현재가']

    #---------------------------------------------주식으로 판단하는 -----------------------------------
    # def judge_stock(self):
    #     col_df_day = self.df_day['현재가']
    #     if col_df_day[0] - col_df_day[1] > 0:
    #         result = '매수'
    #     elif col_df_day[0] - col_df_day[1] == 0:
    #         result = '대기'
    #     else:
    #         result = '매도'
    #     print(result)
    ...
    # 예수금 요청 하기위한 입력값(Set) 서버에 데이터 요청 (commrqdata)
    ...
    def day_receive_stock_info(self, nPrevNext=0):
        self.dynamicCall("SetInputValue(QString, QString)",
                         "종목코드", "005930")
        self.dynamicCall("SetInputValue(QString, QString)", "기준일자", self.date1)
        self.dynamicCall("SetInputValue(QString, QString)", "수정주가구분", "1")
        self.dynamicCall("CommRqData(QString, QString, int, QString)", "주식일봉차트조회",
                         "opt10081", nPrevNext, self.screen_commrqdata_stock)
    ...
    def tr_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        if sRQName == "예수금상세현황요청":
            ...
            for i in range(cnt):
                calculator_list = []  # calculator_list  서버에서 가져온 데이터 값
                ...
                self.cancel_screen_number(self.screen_calculation_stock)
